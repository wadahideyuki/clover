...
abstract class WC_Gateway_PPEC extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'PayPal Express Checkout', 'pp-express-wc4jp' );
		...
		if ( false !== $cert ) {
			...
			if ( false !== $certinfo ) {
				...
				if ( $valid_until < time() ) {
					...
					$expires = '<span style="color: red;">' . __( 'expired on %s', 'pp-express-wc4jp' ) . '</span>';
				} elseif ( $valid_until < ( time() - 2592000 ) ) {
					// Also display in red if the cert is going to expire in the next 30 days
					$expires = '<span style="color: red;">' . __( 'expires on %s', 'pp-express-wc4jp' ) . '</span>';
				} else {
					// Otherwise just display a normal message
					$expires = __( 'expires on %s', 'pp-express-wc4jp' );
				}

				$expires = sprintf( $expires, date_i18n( get_option( 'date_format' ), $valid_until ) );
				$out = sprintf( __( 'Certificate belongs to API username %1$s; %2$s', 'pp-express-wc4jp' ), $certinfo['subject']['CN'], $expires );
			} else {
				$out = __( 'The certificate on file is not valid.', 'pp-express-wc4jp' );
			}
		}
	...
	protected function validate_active_credentials() {
		...
		if ( ! empty( $username ) ) {
			...
			if ( empty( $password ) ) {
				WC_Admin_Settings::add_error( __( 'Error: You must enter API password.', 'pp-express-wc4jp' ) );
				return false;
			}
			...
			if ( is_a( $creds, 'WC_Gateway_PPEC_Client_Credential_Signature' ) && $creds->get_signature() ) {
				...
				try {
					...
					if ( ! $payer_id ) {
						WC_Admin_Settings::add_error( __( 'Error: The API credentials you provided are not valid.  Please double-check that you entered them correctly and try again.', 'pp-express-wc4jp' ) );
						return false;
					}
				} catch ( PayPal_API_Exception $ex ) {

					WC_Admin_Settings::add_error( __( 'An error occurred while trying to validate your API credentials.  Unable to verify that your API credentials are correct.', 'pp-express-wc4jp' ) );
				}
				...
				if ( false === $cert ) {
					WC_Admin_Settings::add_error( __( 'Error: The API certificate is not valid.', 'pp-express-wc4jp' ) );
					return false;
				}
				...
				if ( $valid_until < time() ) {
					WC_Admin_Settings::add_error( __( 'Error: The API certificate has expired.', 'pp-express-wc4jp' ) );
					return false;
				}

				if ( $cert_info['subject']['CN'] != $creds->get_username() ) {
					WC_Admin_Settings::add_error( __( 'Error: The API username does not match the name in the API certificate.  Make sure that you have the correct API certificate.', 'pp-express-wc4jp' ) );
					return false;
				}
				...
				WC_Admin_Settings::add_error( __( 'Error: You must provide API signature or certificate.', 'pp-express-wc4jp' ) );
				return false;
			}
			...
			if ( 'yes' === $settings_array['require_billing'] ) {
				...
				if ( ! $is_account_enabled_for_billing_address ) {
					...
					WC_Admin_Settings::add_error( __( 'The "require billing address" option is not enabled by your account and has been disabled.', 'pp-express-wc4jp' ) );
				}
			}
		}
	}
			...
			<td class="image-component-wrapper">
				<div class="image-preview-wrapper">
					...
					if ( ! $value_is_url ) {
						...
						echo sprintf( __( 'Already using URL as image: %s', 'pp-express-wc4jp' ), $value );
					}
					?>
				</div>
				...
				<button
					...
					data-media-frame-title="<?php echo esc_attr( __( 'Select a image to upload', 'pp-express-wc4jp' ) ); ?>"
					data-media-frame-button="<?php echo esc_attr( __( 'Use this image', 'pp-express-wc4jp' ) ); ?>"
					data-add-image-text="<?php echo esc_attr( __( 'Add image', 'pp-express-wc4jp' ) ); ?>"
					style="<?php echo esc_attr( $maybe_hide_add_style ); ?>"
				>
					<?php echo esc_html__( 'Add image', 'pp-express-wc4jp' ); ?>
				</button>
				...
				>
					<?php echo esc_html__( 'Remove image', 'pp-express-wc4jp' ); ?>
				</button>
