...
class WC_Gateway_PPEC_Checkout_Handler {
	...
	public function paypal_billing_details() {
		...
		?>
		<h3><?php _e( 'Billing details', 'pp-express-wc4jp' ); ?></h3>
		<ul>
			<?php if ( $checkout_details->payer_details->billing_address ) : ?>
				<li><strong><?php _e( 'Address:', 'pp-express-wc4jp' ) ?></strong></br><?php echo WC()->countries->get_formatted_address( $this->get_mapped_billing_address( $checkout_details ) ); ?></li>
			<?php else : ?>
				<li><strong><?php _e( 'Name:', 'pp-express-wc4jp' ) ?></strong> <?php echo esc_html( $checkout_details->payer_details->first_name . ' ' . $checkout_details->payer_details->last_name ); ?></li>
			<?php endif; ?>

			<?php if ( ! empty( $checkout_details->payer_details->email ) ) : ?>
				<li><strong><?php _e( 'Email:', 'pp-express-wc4jp' ) ?></strong> <?php echo esc_html( $checkout_details->payer_details->email ); ?></li>
			<?php endif; ?>

			<?php if ( ! empty( $checkout_details->payer_details->phone_number ) ) : ?>
				<li><strong><?php _e( 'Phone:', 'pp-express-wc4jp' ) ?></strong> <?php echo esc_html( $checkout_details->payer_details->phone_number ); ?></li>
	...
	public function paypal_shipping_details() {
		...
		?>
		<h3><?php _e( 'Shipping details', 'pp-express-wc4jp' ); ?></h3>
		<?php
	...
	public function maybe_cancel_checkout_with_paypal() {
		if ( is_cart() && ! empty( $_GET['wc-gateway-ppec-clear-session'] ) ) {
			...
			$notice =  __( 'You have cancelled Checkout with PayPal. Please try to process your order again.', 'pp-express-wc4jp' );
	...
	public function handle_payment_response( $order, $payment ) {
		...
		if ( 'completed' === strtolower( $payment->payment_status ) ) {
			...
			if ( 'authorization' === $payment->pending_reason ) {
				$order->update_status( 'on-hold', __( 'Payment authorized. Change payment status to processing or complete to capture funds.', 'pp-express-wc4jp' ) );
			} else {
				$order->update_status( 'on-hold', sprintf( __( 'Payment pending (%s).', 'pp-express-wc4jp' ), $payment->pending_reason ) );
			}
			if ( $old_wc ) {
