...
class WC_Gateway_PPEC_Plugin {
	...
	public function bootstrap() {
		try {
			if ( $this->_bootstrapped ) {
				throw new Exception( __( '%s in WooCommerce Gateway PayPal Express Checkout plugin can only be called once', 'pp-express-wc4jp' ), self::ALREADY_BOOTSTRAPED );
			}
	...
	protected function _check_dependencies() {
		if ( ! function_exists( 'WC' ) ) {
			throw new Exception( __( 'WooCommerce Gateway PayPal Express Checkout requires WooCommerce to be activated', 'pp-express-wc4jp' ), self::DEPENDENCIES_UNSATISFIED );
		}

		if ( version_compare( WC()->version, '2.5', '<' ) ) {
			throw new Exception( __( 'WooCommerce Gateway PayPal Express Checkout requires WooCommerce version 2.5 or greater', 'pp-express-wc4jp' ), self::DEPENDENCIES_UNSATISFIED );
		}

		if ( ! function_exists( 'curl_init' ) ) {
			throw new Exception( __( 'WooCommerce Gateway PayPal Express Checkout requires cURL to be installed on your server', 'pp-express-wc4jp' ), self::DEPENDENCIES_UNSATISFIED );
		}

		$openssl_warning = __( 'WooCommerce Gateway PayPal Express Checkout requires OpenSSL >= 1.0.1 to be installed on your server', 'pp-express-wc4jp' );
	...
	protected function _check_credentials() {
		...
		if ( ! is_a( $credential, 'WC_Gateway_PPEC_Client_Credential' ) || '' === $credential->get_username() ) {
			...
			throw new Exception( sprintf( __( 'PayPal Express Checkout is almost ready. To get started, <a href="%s">connect your PayPal account</a>.', 'pp-express-wc4jp' ), esc_url( $setting_link ) ), self::NOT_CONNECTED );
		}
	}
	...
	public function load_plugin_textdomain() {
		load_plugin_textdomain( 'pp-express-wc4jp', false, plugin_basename( $this->plugin_path ) . '/languages' );
	}
	...
	public function plugin_action_links( $links ) {
		...
		if ( function_exists( 'WC' ) ) {
			...
			$plugin_links[] = '<a href="' . esc_url( $setting_url ) . '">' . esc_html__( 'Settings', 'pp-express-wc4jp' ) . '</a>';
		}

		$plugin_links[] = '<a href="https://docs.woocommerce.com/document/paypal-express-checkout/">' . esc_html__( 'Docs', 'pp-express-wc4jp' ) . '</a>';
