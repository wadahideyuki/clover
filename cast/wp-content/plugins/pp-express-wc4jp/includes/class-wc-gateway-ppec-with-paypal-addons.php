	...
	protected function _process_reference_transaction_response( $order, $response ) {
		...
		try {
			if ( ! $client->response_has_success_status( $response ) ) {
				throw new Exception( __( 'PayPal API error', 'pp-express-wc4jp' ) );
			}
			...
			switch ( $status ) {
				case 'Pending':
					...
					$order_note = sprintf( __( 'PayPal transaction held: %s', 'pp-express-wc4jp' ), $response['PENDINGREASON'] );
				...
				case 'In-Progress':
					...
					$order->add_order_note( sprintf( __( 'PayPal payment approved (ID: %s)', 'pp-express-wc4jp' ), $transaction_id ) );
					$order->payment_complete( $transaction_id );
					break;
				default:
					throw new Exception( __( 'PayPal payment declined', 'pp-express-wc4jp' ) );
			}
