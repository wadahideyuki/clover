...
class WC_Gateway_PPEC_Admin_Handler {
	...
	public function add_capture_charge_order_action( $actions ) {
		...
		$actions['ppec_capture_charge'] = esc_html__( 'Capture Charge', 'pp-express-wc4jp' );
	...
	public function show_decimal_warning() {
		if ( get_option( 'wc_gateway_ppce_display_decimal_msg', false ) ) {
			...
			<div class="updated fade">
				<p>
					<strong><?php _e( 'NOTE: PayPal does not accept decimal places for the currency in which you are transacting.  The "Number of Decimals" option in WooCommerce has automatically been set to 0 for you.', 'pp-express-wc4jp' ); ?></strong>
				</p>
			</div>
			<?php
		...
		if ( 'ppec_paypal' === $payment_method ) {
			...
			if ( $trans_id && $this->is_authorized_only( $trans_details ) ) {
				...
				if ( is_wp_error( $result ) ) {
					$order->add_order_note( __( 'Unable to capture charge!', 'pp-express-wc4jp' ) . ' ' . $result->get_error_message() );
				} else {
					...
					$order->add_order_note( sprintf( __( 'PayPal Express Checkout charge complete (Charge ID: %s)', 'pp-express-wc4jp' ), $trans_id ) );
				}
			}
		}
	}
	...
	public function cancel_payment( $order_id ) {
		...
		if ( 'ppec_paypal' === $payment_method ) {
			...
			if ( $trans_id && $this->is_authorized_only( $trans_details ) ) {
				...
				if ( is_wp_error( $result ) ) {
					$order->add_order_note( __( 'Unable to void charge!', 'pp-express-wc4jp' ) . ' ' . $result->get_error_message() );
				} else {
					$order->add_order_note( sprintf( __( 'PayPal Express Checkout charge voided (Charge ID: %s)', 'pp-express-wc4jp' ), $trans_id ) );
				}
			}
		}
	}
