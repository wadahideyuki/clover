<!DOCTYPE html>
<html lang="ja">
<head>
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-50023646-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-50023646-1');
</script>
<meta charset="UTF-8">
<title>募集要項｜株式会社CLOVER</title>
 <meta name="keywords" content="デイサービス,お泊り,介護,居宅,通所,渋谷,新宿,港,千代田,送迎,老人ホーム,ショートステイ" />
<meta name="description" content="株式会社CLOVER（クローバーグループ）の募集要項ページです。新卒、第二新卒、中途、アルバイト・パートを募集しています。私たちは、「人柄・人物重視採用」を行なっています。資格支援制度、研修制度が充実しているので、未経験方でも大歓迎です。" />
<meta http-equiv="Content-Style-Type" content="text/css" />
<meta http-equiv="Content-Script-Type" content="text/javascript" />
<meta name="viewport" content="width=device-width,initial-scale=1">
<meta http-equiv="refresh" content="0;URL=../job/">

<!-- OGs -->
<meta property="og:title" content="募集要項 | 採用情報 | CLOVER" />
<meta property="og:site_name" content="株式会社CLOVER｜都心の365日デイサービス&放課後等デイ" />
<meta property="og:description" content="株式会社CLOVER（クローバーグループ）は、東京都心・千葉県でお泊りデイサービスや、子どもの療育を行う放課後等デイサービスを運営。「Happinessを提供する」を経営理念に、「休日に自宅やカフェでくつろいでいる」ような新しいケア・サービスを目指します。" />
<meta property="og:type" content="website">
<!-- /OGs -->

<link rel="stylesheet" type="text/css" href="../../common/css/style.css">
<link rel="stylesheet" type="text/css" href="../../common/css/class.css">

<script type="text/javascript" src="../../common/js/jquery-1.10.2.min.js"></script>
<script type="text/javascript" src="../../common/js/slick.min.js"></script>
 <?php include('../../common/inc/script.inc');?> 
</head>

<body class="">
<div class="wrapper catCareer pageRecruit">

<!--ヘッダー-->
 <?php include('../../common/inc/header.inc');?>
<!--/ヘッダー-->


<section id="pankz">
	<p><a href="/">クローバー（ホーム）</a>&gt; <a href="/career/index.php">採用情報</a>&gt; <span>募集要項</span></p>
</section>


<section class="">
	<div class="inner">
		<h1 class="ttl2">
			<div>
				<span>募集要項</span>
				<small>Recruit</small>
			</div>
		</h1>
    </div>
  <div class="lowLinks">
		<a href="/career/index.php">採用情報TOP</a>
		<a href="/career/recruit/index.php" class="cur">募集要項</a>
		<a href="/career/welfare/index.php">福利厚生</a>
	</div>
  </section>


<section class="mainImg"><img src="../../common/img/career/recruit/main_img.jpg" alt=""></section>

<div class="bgGrey1">
<section class="statueZone">
	<div class="inner sz2">
		<h2 class="ttl1">求める人物像</h2>
		<ul>
			<li>素直で前向きな人</li>
			<li>気配り、目配りが出来る人</li>
			<li>仲間を大切にする人</li>
			<li>負けず嫌いな人</li>
			<li>行動力のある人</li>
			<li>思いやりのある人</li>
			<li>人の気持ちのわかる<br class="pc">やさしい人</li>
			<li>ちょっと癒し系な人</li>
			<li>楽しい人、面白い人</li>
		</ul>
		<dl>
			<dt>こんな方は<br>お断りします</dt>
			<dd><span>自己中心的で相手の事を考えられない人、</span><span>プラスアルファの仕事をしない人、</span><br class="pc">
				<span>不平、不満、愚痴が多く言い訳をしてやらない人、</span><span>チームの和を乱す人</span></dd>
		</dl>
	</div>
</section>


<section class="bosyuZone">
	<div class="inner sz3">

		<div class="pageScrl type2">
			<a class="clkScrl" href="#scrlSinsotu"><span>新卒・第二新卒の方</span></a>
			<a class="clkScrl" href="#scrlTyuto"><span>中途採用・パート採用の方</span></a>
			<a class="clkScrl" href="https://job.rikunabi.com/2019/company/r152202042/" target="_blank"><span>リクナビ</span></a>
			<a class="clkScrl" href="https://job.mynavi.jp/19/pc/search/corp228103/outline.html" target="_blank"><span>マイナビ</span></a>
		</div>

		<a id="scrlSinsotu" name="scrlSinsotu"></a>
		<div class="tblBox1">
			<h4 class="tblCap">新卒・第二新卒の方</h4>

			<table>
				<tbody>
					<tr>
						<th>職　種</th>
						<td>
							<ul class="numList1">
								<li>【1】デイサービスキャスト</li>
<li>【2】療育スタッフ</li>
<li>【3】経営企画</li>

							</ul>
						</td>
					</tr>
					<tr>
						<th>給　与</th>
						<td>【1】【2】【3】21万円〜
						</td>
					</tr>
					<tr>
						<th>業務内容</th>
						<td>
							<ul class="numList1">
								<li>【1】都内7つに展開する生活リハビリや個別ケア「デイサービスクローバー」でご利用様の介護から見守り、送迎、料理など、自立をサポートします。</li>
<li>【2】「アフタースクールクローバーキッズ麻布十番」でASDやLDなどのさまざまな特性のあるお子さんを放課後等にお預かりし、「得意を探し」「楽しく生きる力」を身につけて、将来のやりたいことの「選択の幅」が増える支援を行います。</li>
<li>【3】経営者視点で今後必要とされてくる新規事業の立案や立ち上げなどを行います。
</li>

							</ul>
						</td>
					</tr>
					<tr>
						<th>資　格</th>
						<td>(共通）思いやりの心のある方<br>
資格、経験不問<br>
<br>

【優遇資格】<br>
・普通自動車免許あれば優遇<br>
・社会福祉士、社会福祉主事任用等、介護福祉士（デイサービスキャスト）<br>
・児童指導員、保育士、児童発達管理者責任者（療育スタッフ）<br>
（文系大学出身者は社会福祉主事任用条件を満たしていることが多いです。詳しくは<a href="/form_recruit/">お問合せ</a>ください。）


						</td>
					</tr>
					<tr>
						<th>時　間</th>
						<td>
							<ul class="noteList1">
								<li>①送迎8：30～17：30</li>
<li>②日勤9：00～18：00</li>
<li>③夜勤18：00～9：00又は22：00～9：00</li>
<li>※デイサービスは夜勤のない事業所もあります。
</li>

							</ul>
						</td>
					</tr>
					<tr>
						<th>休　日</th>
						<td>年間114日　週休2日　シフトによる</td>
					</tr>
					<tr>
						<th>待　遇</th>
						<td>◎交通費支給（上限3万円）<br> 
◎資格取得補助制度 <br>
◎無料での食事支給あり <br>
◎副業、WワークOK！ <br>
◎産休・育休制度あり（過去取得率100％！） <br>
◎キッズルーム利用可 <br>
◎お子様と一緒に出勤OK！
</td>
					</tr>
					<tr>
						<th>応募方法</th>
						<td>お電話にてお問い合わせの上、ご応募下さい。<br>採用担当：03−6457−4343
</td>
					</tr>
				</tbody>
			</table>
      
      <div  class="btnrecruit">
<p class="taC">エントリーはリクナビ、マイナビから！</p>
      <ul>
      <li>
			<a  href="https://job.rikunabi.com/2019/company/r152202042/" target="_blank"><span><img src="/common/img/career/recruit/btn_recnavi.png"></span></a></li>
      <li><a  href="https://job.mynavi.jp/19/pc/search/corp228103/outline.html" target="_blank"><span><img src="/common/img/career/recruit/btn_mynave.png"></span></a></li>
      </ul>
      </div>
		</div>

		<a id="scrlTyuto" name="scrlTyuto"></a>
		<div class="tblBox1">
			<h4 class="tblCap">中途採用・パート採用の方</h4>

			<table>
				<tbody>
					<tr>
						<th>職　種</th>
						<td>
							<ul class="numList1">
								<li>【1】デイサービスキャスト</li>
<li>【2】療育スタッフ</li>
<li>【3】生活相談員</li>
<li>【4】児童発達支援管理責任者</li>
<li>【5】デイサービス管理者候補</li>
<li>【6】機能訓練士</li>
<li>【7】夜勤スタッフ</li>
<li>【8】送迎ドライバー</li>

							</ul>
						</td>
					</tr>
					<tr>
						<th>給　与</th>
						<td>
							<ul class="numList1">
								<li>【1】21万～23万円
                                    時給1,000円～</li>
<li>【2】21万～23万円
                    時給1,000円～</li>
<li>【3】23万～27万円
                    時給1,100円～</li>
<li>【4】27万〜35万円</li>
<li>【5】27万〜35万円</li>
<li>【6】時給1，500円</li>
<li>【7】18:00～9:00 日給15,500円<br>20:00～9:00 日給12,000円<br>22:00～9:00 日給10,000円</li>
<li>【8】時給1,000円</li>

							</ul>
						</td>
					</tr>
					<tr>
						<th>業務内容</th>
						<td>
							<ul class="numList1">
								<li>【1】都内7つに展開する生活リハビリや個別ケア「デイサービスクローバー」でご利用様の介護から見守り、送迎、料理など、自立をサポートします</li>
<li>【2】「アフタースクールクローバーキッズ麻布十番」でASDやLDなどのさまざまな特性のあるお子さんを放課後等にお預かりし、「得意を探し」「楽しく生きる力」を身につけて、将来のやりたいことの「選択の幅」が増える支援を行います。</li>
<li>【3】都内7つに展開する生活リハビリや個別ケア「デイサービスクローバー」でご利用様の介護から見守り、送迎、料理など、通所介護計画書の作成、ご家族やケアマネージャーとの対応を行います。</li>
<li>【4】「アフタースクールクローバーキッズ麻布十番」でASDやLDなどのさまざまな特性のあるお子さんを放課後等にお預かりし、アセスメントの作成、個別支援計画、相談業務を行います。</li>
<li>【5】都内7つに展開する生活リハビリや個別ケア「デイサービスクローバー」でご利用様の介護から見守り、送迎、料理など、通所介護計画書の作成、ご家族やケアマネージャーとの対応、スタッフの育成を行います。</li>
<li>【6】機能体操などのプログラム作成、スタッフ指導</li>
<li>【7】夜の見守り、食事介助、トイレ介助など</li>

							</ul>
						</td>
					</tr>
					<tr>
						<th>資　格</th>
						<td>(共通）思いやりの心のある方<br>(共通）普通自動車免許、夜勤が出来る方は優遇<br><br>
                            <ul class="numList1">
                            <li>【1】未経験、資格なくても歓迎</li>

<li>【2】◆有資格者は未経験歓迎！<br>◆資格がない方は2年以上の児童福祉の経験がある方<br>
下記いずれかの要件を満たす方 <br>
◎児童福祉施設の職員を養成する学校その他の養成施設を卒業した方 <br>
◎社会福祉士、精神保健福祉士の有資格者 <br>
◎大学の学部で社会福祉学、心理学、教育学もしくは社会学を専修する学科 <br>
又はこれらに相当する課程を修めて卒業した方 
◎小学校、中学校、義務教育学校、高等学校又は中等教育学校の教諭資格を有する方 
◎大学を卒業して2年以上児童福祉事業に従事した方 </li>
<li>【3】社会福祉士、社会福祉主事任用、介護福祉士等<br>
（文系大学出身者は社会福祉主事任用条件を満たしていることが多いです。詳しくはお問合せください。）</li>
<li>【4】児童発達支援管理責任者の研修対象者。実務未経験OK。<br>
【具体的には】<br>
児童発達支援管理責任者の資格は下記の経験がある方が研修を受講することで取得可能。  <br>
■社会福祉士、作業療法士、理学療法士、精神保健福祉士などの国家資格保有者  <br>
■児童指導員任用資格者（臨床心理士を含む）  <br>
■児童養護施設での勤務経験  <br>
■学校（小・中・高）での勤務経験、教員免許保有、特別支援学校・支援学級での経験  <br>
■幼稚園、保育所、保育園での勤務経験  <br>
■障害福祉サービスに携わった経験  <br>
■就労支援事業所での勤務経験 <br>
■児童相談所で働いた経験  <br>
■障害者支援施設で働いた経験  <br><br>

上記の資格を持って、5～10年の経験が必要。 <br><br>

※児発管には「相談支援業務」「直接支援業務」「国家資格および従事経験」 いずれかの要件が求められます。 要件の詳細については、厚労省や各自治体のHP等でご確認頂けます。 </li>

<li>【5】社会福祉士、社会福祉主事任用、介護福祉士等
（文系大学出身者は社会福祉主事任用条件を満たしていることが多いです。詳しくは<a href="/form_recruit/">お問合せ</a>ください。）</li>
<li>【6】機能訓練士
（看護師、OT、PT、言語聴覚士、柔道整復師、あん摩マッサージ指圧師）</li>
<li>【7】特になし</li>
                            </ul>


						</td>
					</tr>
					<tr>
						<th>時　間</th>
						<td>
							<ul class="noteList1">
								<li>①送迎8：30～17：30</li>
<li>②日勤9：00～18：00</li>
<li>③夜勤18：00～9：00</li>
<li>④機能訓練士は週1回2時間～3時間程度　状況により変更あり</li>
<li>※デイサービスは夜勤のない事業所もあります。</li>

							</ul>
						</td>
					</tr>
					<tr>
						<th>休　日</th>
						<td>年間114日　週休2日　シフトによる</td>
					</tr>
					<tr>
						<th>待　遇</th>
						<td>◎交通費支給（上限3万円） <br>
◎資格手当 <br>
◎資格取得補助制度 <br>
◎無料での食事支給あり <br>
◎副業、WワークOK！ <br>
◎産休・育休制度あり（過去取得率100％！） <br>
◎キッズルーム利用可 <br>
◎お子様と一緒に出勤OK！
</td>
					</tr>
					<tr>
						<th>応募方法</th>
						<td>お電話にてお問い合わせの上、ご応募下さい。<br>採用担当：03−6457−4343
</td>
					</tr>
				</tbody>
			</table>

			<div class="btnWrap">
				<a class="btn clr2 sz2 withArwR" href="/form_recruit/index.php">エントリーはこちら</a>
			</div>
		</div>
	</div>
</section>
</div>


<!--フッター-->
 <?php include('../../common/inc/footer.inc');?>
<!--/フッター-->

</div><!--/.wrapper-->
</body>
</html>
